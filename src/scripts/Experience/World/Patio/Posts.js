...
const postsFrontGroup = new THREE.Group()
const postsBackGroup = new THREE.Group()
patioGroup.add(postsFrontGroup, postsBackGroup)
postsFrontGroup.position.z = - (PARAMS.roofDepth / 2 - 1)
postsBackGroup.position.z = PARAMS.roofDepth / 2 - 1

// Post asset 
const postSide01 = new THREE.Mesh(
    new THREE.BoxGeometry(PARAMS.postsSizes.widthSide, PARAMS.postsSizes.height, PARAMS.postsSizes.depthSide),
    materials.posts
)
const postSide02 = new THREE.Mesh(
    new THREE.BoxGeometry(PARAMS.postsSizes.widthSide, PARAMS.postsSizes.height, PARAMS.postsSizes.depthSide),
    materials.posts
)
const postCenter = new THREE.Mesh(
    new THREE.BoxGeometry(PARAMS.postsSizes.widthCenter, PARAMS.postsSizes.height, PARAMS.postsSizes.depthCenter),
    materials.posts
)
postSide01.position.z = - (PARAMS.postsSizes.depthCenter / 2 + PARAMS.postsSizes.depthSide / 2)
postSide02.position.z = PARAMS.postsSizes.depthCenter / 2 + PARAMS.postsSizes.depthSide / 2

const postAsset = new THREE.Group()
postAsset.add(postSide01, postSide02, postCenter)
postAsset.traverse((child) => {
    if (child.isMesh) {
        child.receiveShadow = true
        child.castShadow = true
    }
...
let this.postsStatus = null

function numberOfPosts() {

    let totalWidth, maxDistance, maxBlocks
    ...
    if (PARAMS.patioType === 'solid' || PARAMS.patioType === 'insulated') {
        ...
        if (this.postsStatus === '01') {
            if (PARAMS.roofDepth === 10 && PARAMS.roofDepth < 11) {
                totalWidth = PARAMS.roofWidth
                maxDistance = 8.58 // 8.7 ft
            }
            if (PARAMS.roofDepth === 11 || PARAMS.roofDepth < 12) {
                totalWidth = PARAMS.roofWidth
                maxDistance = 8.167 // 8.2 ft
            }
            if (PARAMS.roofDepth === 12) {
                totalWidth = PARAMS.roofWidth
                maxDistance = 7.667 // 7.8 ft
            }
        }
        if (this.postsStatus === '02') {
            if (PARAMS.roofDepth === 10 && PARAMS.roofDepth < 11) {
                totalWidth = PARAMS.roofWidth
                maxDistance = 12.33 // 12.4 ft
            }
            if (PARAMS.roofDepth === 11 || PARAMS.roofDepth < 12) {
                totalWidth = PARAMS.roofWidth
                maxDistance = 11.33 // 11.4 ft
            }
            if (PARAMS.roofDepth === 12) {
                totalWidth = PARAMS.roofWidth
                maxDistance = 10.417 // 10.5 ft
            }
        }

        maxBlocks = Math.floor(totalWidth / maxDistance)
        PARAMS.postsNumber = 2 + maxBlocks
    ...
    if (PARAMS.patioType === 'lattice') {
        if (PARAMS.roofDepth === 10 && PARAMS.roofDepth < 11) {
            totalWidth = PARAMS.roofWidth - 2
            maxDistance = 10.167 // 10.2 ft
        }
        if (PARAMS.roofDepth === 11 || PARAMS.roofDepth < 12) {
            totalWidth = PARAMS.roofWidth - 2
            maxDistance = 9.667 // 9.8 ft
        }
        if (PARAMS.roofDepth === 12) {
            totalWidth = PARAMS.roofWidth - 2
            maxDistance = 9.167 // 9.2 ft
        }
        maxBlocks = Math.floor(totalWidth / maxDistance)
        PARAMS.postsNumber = 2 + maxBlocks
...
function createPosts() {
    ...
    for (let i = 0; i < PARAMS.postsNumber; i++) {
        ...
        const postBack = postAsset.clone()
        postBack.position.x = - (PARAMS.roofWidth - 2) / 2 + i * margin
        postsBackGroup.add(postBack)

        const postFront = postAsset.clone()
        postFront.position.x = - (PARAMS.roofWidth - 2) / 2 + i * margin
        postsFrontGroup.add(postFront)
    }
}

function deletePosts() {
    while (postsBackGroup.children[0]) {
        postsBackGroup.remove(postsBackGroup.children[0])
    }
    while (postsFrontGroup.children[0]) {
        postsFrontGroup.remove(postsFrontGroup.children[0])
    }
}

function updatePostsAndBeams() {
    deletePosts()
    if (PARAMS.postsSizes.diametr) {
        postSide01.geometry = new THREE.BoxGeometry(PARAMS.postsSizes.widthSide, PARAMS.postsSizes.height, PARAMS.postsSizes.depthSide)
        postSide02.geometry = new THREE.BoxGeometry(PARAMS.postsSizes.widthSide, PARAMS.postsSizes.height, PARAMS.postsSizes.depthSide)
        postCenter.geometry = new THREE.CylinderGeometry(PARAMS.postsSizes.diametr, PARAMS.postsSizes.diametr, PARAMS.postsSizes.height, 32, 4)
    }
    if (PARAMS.postsSizes.widthCenter) {
        postSide01.geometry = new THREE.BoxGeometry(PARAMS.postsSizes.widthSide, PARAMS.postsSizes.height, PARAMS.postsSizes.depthSide)
        postSide02.geometry = new THREE.BoxGeometry(PARAMS.postsSizes.widthSide, PARAMS.postsSizes.height, PARAMS.postsSizes.depthSide)
        postCenter.geometry = new THREE.BoxGeometry(PARAMS.postsSizes.widthCenter, PARAMS.postsSizes.height, PARAMS.postsSizes.depthCenter)
    }
    createPosts()
